...
class RagModelCallMetricServiceTests {
    ...
    void getMetricsShouldAggregateByWorkspaceModelAndPromptVersion() {
        ...
        repository.save(observation("5", 1L, "gpt-test", "rag-chat-v2", RagModelCallStatus.CANCELLED, 500));
        repository.save(observation("6", 2L, "other-model", "rag-chat-v1", RagModelCallStatus.SUCCEEDED, 50));

        RagModelCallMetricsResponse response = service.getMetrics(1L);

        assertThat(response.workspaceId()).isEqualTo(1L);
        assertThat(response.totalCallCount()).isEqualTo(5);
        ...
        assertThat(response.cancelledCallCount()).isEqualTo(1);
        assertThat(response.successRate()).isEqualByComparingTo("0.4000");
        assertThat(response.fallbackRate()).isEqualByComparingTo("0.2000");
        assertThat(response.averageElapsedMillis()).isEqualByComparingTo("300.00");
        ...
        assertThat(fallbackGroup.totalCallCount()).isEqualTo(3);
        assertThat(fallbackGroup.cancelledCallCount()).isEqualTo(1);
        assertThat(fallbackGroup.successRate()).isEqualByComparingTo(BigDecimal.ZERO);
        assertThat(fallbackGroup.fallbackRate()).isEqualByComparingTo("0.3333");
        assertThat(fallbackGroup.averageElapsedMillis()).isEqualByComparingTo("400.00");
    }
    ...
    void getMetricsShouldReturnZeroValuesWhenWorkspaceHasNoCalls() {
        ...
        assertThat(response.cancelledCallCount()).isZero();
