...
本文档说明如何用可重复的本地环境验证 AgentMind 的第一条前后端完整业务链路。该测试使用本地 JWT、内存存储、确定性向量和模拟回答生成器，不依赖数据库、外部模型密钥或真实用户数据。
...
3. 通过前端注册真实测试账号并获得本地 JWT。
4. 从采集中心上传固定 Markdown 资料。
5. 验证摄取任务成功，并在知识库列表中看到文档、类型和状态。
6. 从 Agent 问答页面携带令牌发起 SSE 流式问题。
7. 验证回答来自后端检索增强链路，并返回上传文档的引用来源。
8. 验证浏览器没有未处理异常。
...
测试配置会显式设置 `AGENTMIND_SECURITY_ALLOW_MEMORY_FOR_TESTING=true`，仅用于在无 PostgreSQL 的
自动化环境验证 JWT 浏览器流程。该开关默认关闭，local 和 production 不得启用。
