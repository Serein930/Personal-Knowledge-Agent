...
会话删除同样使用 Lua 脚本，一次性删除会话正文、消息哈希、消息顺序，并移除知识空间会话索引成员；索引为空时会连同空索引一起清理。
...
- 重命名、归档和删除与内存适配器遵循同一仓储契约。
- 归档后禁止创建新消息，但允许已经等待中的助手消息完成。
...
- 当前删除会物理清理 Redis 短期记忆，不提供恢复功能。
- 当前 Token 计算默认采用 Spring AI JTokkit，其他模型编码需要替换计算适配器。
